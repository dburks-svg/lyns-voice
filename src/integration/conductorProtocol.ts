...
/**
 * The spawn gate's path check: is `child` the same directory as `root` or inside
 * it? A `<<spawn>>` whose directory stays within the primary session's project dir
 * keeps the user-chosen blast radius and may run unprompted; anywhere else must be
 * confirmed by the user (a prompt-injected reply could otherwise point a worker at
 * an arbitrary directory). Windows semantics: case-insensitive, / and \ equal.
 * `..` segments are rejected outright rather than resolved - this runs in the
