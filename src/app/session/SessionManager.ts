...
export interface SessionManagerDeps {
  ...
  /** MCP servers disabled in the Library (read live at spawn); workers inherit the
   *  same MCP world as the primary. */
  mcpDisabled?: () => string[];
  /** Hook ids disabled in the Library (read live at spawn); same inheritance. */
...
export class SessionManager {
  ...
  async spawn(opts?: {
    ...
    const disabledMcp = this.deps.mcpDisabled?.() ?? [];
    if (disabledMcp.length) args.disabledMcp = disabledMcp;
    const disabledHooks = this.deps.hooksDisabled?.() ?? [];
