...
LYNS Voice is distributed under the [MIT License](LICENSE). It builds on, bundles, and (at first run)
...
Every third-party component used by LYNS Voice is under a permissive, MIT-compatible license
(MIT, Apache-2.0, BSD, ISC, Zlib, Unicode-3.0, CC0, the Unlicense, WTFPL, CC-BY-3.0, or
MPL-2.0).
...
- Component: `jarvis-ai-orb-web-animation`, vendored under `src/avatar/orb/`
  (its renderer ported to Three.js r128).
- Author: cyber1443 (https://github.com/cyber1443/jarvis-ai-orb-web-animation)
- License: MIT. Full text: `src/avatar/orb/LICENSE`
...
These are not committed to the repository. LYNS Voice downloads them once on first use (over HTTPS,
...
### Whisper STT model (`ggml-base.en-q5_1`) - MIT
...
The speech crates above compile and statically link C/C++ libraries whose licenses
apply to the shipped binary independently of the wrapper crate's license:

- **whisper.cpp and ggml - MIT.** `whisper-rs-sys` (itself Unlicense) builds and links
  whisper.cpp and its ggml runtime. Copyright (c) 2023-2024 Georgi Gerganov and the
  ggml authors. https://github.com/ggerganov/whisper.cpp
- **libfvad - BSD-3-Clause.** `webrtc-vad` (itself MIT) builds and links libfvad, the
  standalone extract of the WebRTC voice-activity detector. Copyright (c) 2011, The
  WebRTC project authors; standalone packaging Copyright (c) 2016, Daniel Pirch.
  BSD-3-Clause requires this attribution be preserved in binary distributions.
  https://github.com/dpirch/libfvad

### misaki-rs lexicon data

`misaki-rs` bundles English grapheme-to-phoneme lexicon data derived from the
Python `misaki` project by hexgrad (https://github.com/hexgrad/misaki, Apache-2.0),
the G2P engine written for Kokoro. The crate code is MIT; the bundled lexicon data
carries the upstream Apache-2.0 provenance noted here.
...
feature off to preserve LYNS Voice's permissive-only dependency tree.
...
- **`r-efi`** is tri-licensed (`MIT OR Apache-2.0 OR LGPL-2.1-or-later`); LYNS Voice uses it under
  MIT/Apache-2.0, not the LGPL option.
- **`whisper-rs` / `whisper-rs-sys`** are released into the public domain (Unlicense).
- **`language-tokenizer`** (0.1.0, pulled in by `misaki-rs`) is licensed under the
  **WTFPL** (version 2), declared only in its bundled `LICENSE.md` (no SPDX field in its
  crate metadata, so machine-generated BOMs may report it as unlicensed). WTFPL is a
  do-anything permissive license; no attribution is required, noted here for completeness.
...
- BSD-2-Clause / BSD-3-Clause, ISC, Zlib, Unicode-3.0, CC0-1.0, BSL-1.0, Unlicense,
  WTFPL, CDLA-Permissive-2.0: see the corresponding SPDX identifier at
