# CLAUDE.md - LYNS Voice (Tauri desktop app)
...
app's renderer is `OrbAvatar` (the vendored Three.js orb); the demo keeps the legacy
...
- **FUI + Command Center:** the vendored MIT Three.js orb centerpiece (full-window) + the
  three-column tactical HUD + live telemetry panels (transcript / activity / session
  tokens+cost / mic waveform); three switchable themes; an "Oracle" wake word with an always-on
...
src/
  ...
  avatar/
    OrbAvatar.ts        Adapter: drives the vendored orb through the ControllableAvatar seam
    orb/                Vendored MIT Three.js orb (renderer.ts + states.ts; keep its LICENSE)
  ...
  integration/
    ...
    replyStreamer.ts, wakeWord.ts, voices.ts  Sentence streaming, the "Oracle" wake word, voice labels
...
- **Frontend seam** (`tauriAdapter.ts`): the only substantial glue. It wires the events to the
  ...
  watchdog, and injects the renderer via `avatarFactory` (default `OrbAvatar`).
