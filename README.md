...
![macOS](https://img.shields.io/badge/macOS-on_the_way-9e9e9e?logo=apple&logoColor=white)
...
![Whisper](https://img.shields.io/badge/STT-Whisper_(local)-4b8bbe)
![Kokoro](https://img.shields.io/badge/TTS-Kokoro_(local)-8a2be2)
![License: MIT](https://img.shields.io/badge/License-MIT-green)

A standalone desktop app (Tauri v2) that is the voice of Claude Code - part of the
[LYNS](https://lyns.dev) family, where the one you speak with is **Oracle**. Speak a
...
![Oracle speaking a reply: the holographic orb, live caption, session window, and the Library of MCP servers and hooks](docs/screenshots/speaking.png)
...
- **"Oracle" wake word (optional):** say "Oracle, <command>" (or "hey Oracle") - keeps the mic armed so you can start a turn without a tap.
...
- A **holographic orb** (Three.js) centerpiece on a three-column tactical HUD; the
...
- **Live telemetry panels** (draggable, resizable, snap-to-edge), fed by the voice loop:
  - **Transcript** (you and Oracle, with right-click copy and session persistence)
...
- **Compact PiP mode:** shrink the app to a tiny 180x180 always-on-top orb window. Click restore
  or press Alt+M to return to full size.
- **Auto-reconnect:** if the Claude sidecar exits unexpectedly, LYNS Voice retries with exponential
...
- **System notifications:** when LYNS Voice is backgrounded and Claude finishes a turn, a Windows
...
The `claude` CLI must be on your PATH at runtime. **Windows only for now**; a macOS port
is on the way (the speech stack, terminal layer, and UI are already cross-platform crates).
...
This produces an unsigned per-user NSIS installer (`LYNS Voice_<version>_x64-setup.exe`) plus the
...
src/
  ...
  avatar/
    OrbAvatar.ts           Adapter: drives the orb through the ControllableAvatar seam
    orb/                   Vendored MIT Three.js orb (renderer.ts + states.ts; see its LICENSE)
...
orb-adapter bridge, keyboard shortcuts, panel snap-to-edge, the session panel + manager +
...
- The holographic orb is the MIT-licensed
  ...
  cyber1443, vendored under `src/avatar/orb/` (its renderer ported to Three.js r128).
...
LYNS Voice is released under the [MIT License](LICENSE). It bundles and downloads third-party
components under their own permissive licenses (MIT / Apache-2.0 / CC-BY-3.0); see the
**Acknowledgements** above, `vendor/NOTICE.md`, and `src/avatar/orb/LICENSE`.
