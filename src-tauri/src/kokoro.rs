...
impl Synthesizer {
    ...
    fn synthesize(&self, text: &str, voice: &str, speed: f32) -> Result<Vec<u8>, String> {
        ...
        let mut ids: Vec<i64> = phonemes
            .chars()
            .filter_map(|c| self.vocab.get(&c).copied())
            .collect();
        ids.truncate(MAX_PHONEME_TOKENS);
        if ids.is_empty() {
            return Err("no pronounceable tokens for input".into());
        }
        let n_tokens = ids.len();

        // 3. style vector for this token count (Kokoro selects style by sequence len).
        let style = self.voice_style(voice, n_tokens)?;

        // 4. pad token 0 at start AND end (Kokoro's expected input framing).
        let mut input_ids = Vec::with_capacity(n_tokens + 2);
        input_ids.push(0);
        input_ids.extend_from_slice(&ids);
        input_ids.push(0);

        // 5. run inference -> f32 samples in [-1, 1] at 24 kHz.
        let samples = self.infer(&input_ids, &style, speed)?;

        // 6. f32 -> little-endian i16 PCM, then wrap with the shared RIFF header.
